# ruby spectra_counts_summary_tables_common-like_n_filtered.rb ../results/phospho_soft_stiff_b1_b2_summaries_shortened2_.csv
...
# create filtered lists for peptides for each condition with count >2 in both experiments
cond1_filtered = Hash.new {|h,k| h[k] = [] }
condition1_mod_pep_hits.each_key do |peptide|
	if (condition1_mod_pep_hits[peptide][2].to_i > 2 || condition2_mod_pep_hits[peptide][2].to_i > 2) && (condition3_mod_pep_hits[peptide][2].to_i > 2 || condition4_mod_pep_hits[peptide][2].to_i > 2)
